...
impl<'a> HuyaLive<'a> {
    ...
    async fn check_stream(&self) -> LiveResult<LiveStatus> {
        ...
        let stream_headers = if self.should_use_wup() {
            HashMap::from([("User-Agent".to_string(), huya_wup::WUP_UA.to_string())])
        } else {
            HashMap::new()
        };
    ...
    async fn build_stream_urls(&self, streams_info: &[Value]) -> LiveResult<Vec<(String, String)>> {
        ...
        for stream in streams_info {
            ...
            if cached_anticode.is_none() {
                ...
                // 其余情况：use_wup=true 走 WUP，失败回退页面防盗链；use_wup=false
                // 直接用页面 anti_code + lPresenterUid 重建。
                ...
                let anticode = if self.huya_mobile_api && self.huya_imgplus {
                    ...
                    match self.get_cdn_token_info_ex(&stream_name).await {
                        ...
                        Err(err) => {
                            ...
                            build_anticode(&stream_name, page_anti_code, presenter_uid)?
                        }
                    }
                } else {
                    build_anticode(&stream_name, page_anti_code, presenter_uid)?
...
fn should_use_wup(mobile_api: bool, imgplus: bool, use_wup: bool) -> bool {
    // mobile API + imgplus keeps raw anti_code; otherwise honor use_wup.
...
mod tests {
    ...
    fn should_use_wup_respects_flag_when_not_mobile_imgplus() {
        ...
        assert!(!should_use_wup(true, false, false));
    }
}
